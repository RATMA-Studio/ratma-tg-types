...
Auto-generated full serde-based and async Telegram Bot API wrapper.

Maps Telegram types and methods cleanly onto Rust design patterns while keeping overhead low and avoiding unnecessary clones or allocations. Methods take advantage of generic parameters, slices, and primitive types whenever possible; all API types come with an ergonomic builder-pattern interface.
...
## Adding ratma-tg-types to your project
You can add via cargo-edit

```bash
cargo add ratma-tg-types
```
...
```toml
[dependencies]
ratma-tg-types = { version = "0.0.79", features = [] }
```
...
all ratma-tg-types functions. If you need a T from a BoxWrapper just
...
for all telegram types. Yes, this needs to be integrated with the ratma-tg-types
...
```toml
[dependencies]
ratma-tg-types = { version = "0.0.79", features = [ "rhai" ] }
```

### Initializing rhai engine
In order to interact with ratma-tg-types types in a rhai environemnt, each
...
// Register all types
ratma_tg_types::gen_types::rhai_helpers::setup_all_rhai(&mut engine);
...
Most fields in ratma-tg-types types are accessible directly to rhai scripts. The
main exception is enums, which are exposed using an api similar to

Example checking enum fields from a `ratma_tg_types::gen_types::Message` type
the `m` parameter to the anonymous function is assumed to be `ratma_tg_types::gen_types::Message`
```rust
...
Example checking if a `ratma_tg_types::gen_types::Message` has text
```rust
...
Docs are live at [https://docs.rs/ratma-tg-types](https://docs.rs/ratma-tg-types),
